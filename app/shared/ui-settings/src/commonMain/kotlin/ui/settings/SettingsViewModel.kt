...
class SettingsViewModel : AbstractSettingsViewModel(), KoinComponent {
    ...
    // Probes PikPak auth through the engine that actually serves playback and
    // caching, so a green Test means the same client, credentials and session
    // store the engine will use really work. The engine keeps the password
    // persisted (obscured, see PikPakConfig.password) so a revoked refresh
    // token can be recovered without prompting the user; an existing refresh
    // token is also a usable auth path on its own. NOT_ENABLED therefore
    // requires both credential fields blank, not just the password.
    val pikpakConnectionTester: ConnectionTester = ConnectionTester(id = "pikpak") {
        ...
        } else if (pikpakEngine.testConnection()) {
