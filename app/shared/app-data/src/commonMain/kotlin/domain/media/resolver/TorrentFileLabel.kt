 ...
 * 615 个发布组, 见 docs/pikpak-ani-design.md 的记录. 判别顺序有讲究:
...
internal data class TorrentFileLabel(val kind: Kind, val number: Int?) {
    ...
    companion object {
        ...
        fun of(fileName: String): TorrentFileLabel? {
            ...
            // 按它判会把整包连正片一起当成花絮. 但单看一条路径分不出第一段是不是根目录 (anitorrent 的
            // pathInTorrent 带根目录, PikPak 的不带), 所以剥根目录是 TorrentMediaResolver 拿到整份清单
            // 之后的事, 这里只跳过带加号的目录, 那同样是内容清单的写法.
