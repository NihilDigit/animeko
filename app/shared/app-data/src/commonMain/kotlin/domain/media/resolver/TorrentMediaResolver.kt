...
class TorrentMediaResolver(
    ...
    companion object {
         ...
         * 剥掉所有条目共有的首段目录.
         *
         * anitorrent 的 pathInTorrent 带种子根目录 (libtorrent 的 file_path), PikPak 的不带 (walkBucket
         * 剥掉了第一层文件夹). 根目录名列的是整包的内容清单, `Title - TV + OVA + SP` 末尾正好是 SP,
         * 让它参与类别判定会把整包连正片一起当成花絮. 按「所有文件共有」而不是「第一段」判:
         * anitorrent 下根目录必然共有, PikPak 下没有根目录也就没有共有段, 两边由此对齐.
         */
