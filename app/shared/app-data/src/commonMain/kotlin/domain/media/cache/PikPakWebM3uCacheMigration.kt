...
class PikPakWebM3uCacheMigration(
    ...
    suspend fun migrate() {
        ...
        // metadata 已经写回, 迁移到此才算提交, 旧链路那几行现在可以删了. 删失败无所谓: 下次启动
        // 这些记录已经不是 WebM3u, 不会再进候选, 残留的行只是几条没人读的数据.
    ...
    private suspend fun migrateOne(origin: Media): MigratedFile {
        ...
        // 上一次迁移搬完文件、还没写回 metadata 就退出的情形: 源文件已经不在原处, 但目标已经就位.
        // 不认出这一步, 重试会把一条已完成的缓存判成没下完, 连带把 torrent 行改成 completed=false.
        ...
        // 旧链路的那行不在这里删: 它是这次迁移唯一的恢复信息, 而迁移的提交点是 [migrate] 末尾
        // 那一次 metadata 写入. 在提交之前删掉, 中途退出就会留下一条自称 WebM3u、文件却已经搬走、
        // 又没有 HTTP 状态可查的记录.
