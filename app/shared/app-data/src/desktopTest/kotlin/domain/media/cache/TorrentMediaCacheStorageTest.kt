...
class TorrentMediaCacheStorageTest : AbstractTorrentMediaCacheEngineTest() {
    ...
    fun `auto cache follows playback while full download is off`() = runTest {
        val storage = createStorage(
            createEngine(
                fullDownloadForAutoCaches = flowOf(false),
    ...
    @Test
    fun `auto cache upgrades to full download when the flag flips on`() = runTest {
        val flag = MutableStateFlow(false)
        val storage = createStorage(
            createEngine(
                fullDownloadForAutoCaches = flag,
                onDownloadStarted = { it.onTorrentChecked() },
            ),
        )
    ...
    /**
     * 缓存页上的恢复按钮表达「我要这一条」, 之后这条记录不再随 Reseeding 开关摆动.
     */
    @Test
