...
class PikPakCacheServiceController(
    ...
    fun start() {
        scope.launch {
            ...
            ) { shouldRun, processState, timedOut -> Triple(shouldRun, processState, timedOut) }
                .distinctUntilChanged()
                .onEach { (shouldRun, processState, timedOut) ->
                    if (timedOut) {
                        stopService()
                    ...
                    when {
                        shouldRun && !serviceStarted -> startService()
                        !shouldRun && serviceStarted -> stopService()
                    }
                }
                .collect()
        }
    ...
    private fun startService() {
        try {
            ...
            serviceStarted = true
            logger.info { "PikPakCacheService started." }
        } catch (e: IllegalStateException) {
            // ForegroundServiceStartNotAllowedException on Android 12+. Leaving serviceStarted
            // false is what makes the next process-lifecycle change retry.
