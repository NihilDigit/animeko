...
class PikPakTorrentDownloader(
    ...
    override fun close() {
        ...
        // Snapshot before cancelling: creation still in flight dies with scope.
        val live = sessions.values.mapNotNull { completedSessionOf(it) }
        cleanupScope.launch {
    ...
    private suspend fun account(): PikPakAccount = clientLock.withLock {
        ...
        PikPakClient(
            ...
            // meaning anything. Widening the OkHttp cap to match instead was measured worse: the
            // link is the limit, and more connections only slice the same bandwidth thinner.
...
internal class PikPakAccount(val client: PikPakClient, scope: CoroutineScope) {
    ...
    val startupSweep = scope.launch {
        try {
            ...
            // An object is useful only between its creation and the link minted from it, measured
            // in hundreds of milliseconds, so five minutes is already generous towards another
            // device midway through that. Taking one early is not damage either: the reader finds
            // a 404 on the detail and rebuilds, which is the ladder the SDK is built around. A day
            // was the cautious first guess, and it meant anything that did leak stayed for a day.
            ...
            // Only the failure used to say anything, so a sweep that did collect something was
            // indistinguishable from one that found nothing to collect.
