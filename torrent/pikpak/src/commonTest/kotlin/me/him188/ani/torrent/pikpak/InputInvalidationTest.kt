...
class InputInvalidationTest {
    ...
    fun `a fill that picks the disk after the file was unlinked does not fail unexpectedly`() {
        ...
        try {
            ...
            // Measured on Windows: this throws. An open read handle blocks the unlink, so the state
            // the window would need -- name gone, handle live, pieces still finished -- cannot be
            // reached there at all, and deleteTarget would fail before ever reaching its piece
            // reset. Where the unlink does succeed, the handle keeps the bytes readable.
            ...
            // What actually happens on every platform measured: the unlink removes the name, the
            // handle opened before it keeps the bytes readable, and the fill is served correctly.
            // That is why the reordering this window seemed to call for was not worth making.
